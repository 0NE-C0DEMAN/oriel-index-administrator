...
def generate_scaletrader_ticket(
    ...
    *,
    ...
    Direction convention:
      The YES leg pays when the threshold is breached in the contract's
      stated direction (above or below). Cheap YES means the market is
      pricing too low a probability of that breach vs Oriel FV.

      - For an 'above X%' contract: negative dislocation (implied_yoy
        below Oriel FV) means the market under-prices the chance of CPI
        going above X, so YES is cheap -> Buy YES.
      - For a 'below X%' contract: the same negative dislocation means
        the market is over-pricing the chance of CPI going below X, so
        YES is rich -> Sell YES.

      The pre-direction-fix harness assumed every contract was 'above'
      and flipped side purely on dislocation sign, which inverted every
      ticket for 'below' markets.
    ...
    direction = str(data.get("threshold_direction") or "above").strip().lower()
    ...
    # For a 'below' contract the YES leg pays on the opposite breach, so
    # the cheap/rich sign of the same dislocation flips. Easiest to model
    # as a single boolean: do we want to Buy YES on this row?
    buy_yes = (dislocation_bps < 0) if direction == "above" else (dislocation_bps > 0)
